...
<?php
	...
	?>
				...
				<td><?php echo '<a class = "btn btn-primary" href = "?pettyac=a&petty_id='.$row['petty_id'].'">Approve</a> ';
						echo '<a class = "btn btn-primary" href = "?pettyac=d&petty_id='.$row['petty_id'].'"">Disapprove</a>';?></td>
				</tr>
	<?php
		}
...
<?php
	if(isset($_GET['pettyac']) && $_GET['pettyac'] == 'a'){
		...
		echo '<table align = "center" class = "table table-hover" style = "width: 800px;">';
	...
	if($result->num_rows > 0){
		while($row = $result->fetch_assoc()){
			echo '<tr><td style = "width: 50%;">Date: </td><td style = "width: 50%;">' . date("F j, Y", strtotime($row['date'])).'</td></tr>';
			echo '<tr><td style = "width: 50%;">Petty Number: </td><td style = "width: 50%;"><input name = "petty_id"type = "hidden" value = "' . $row['petty_id'].'"/>' . $row['petty_id'].'</td></tr>';
			echo '<tr><td style = "width: 50%;">Name : </td><td style = "width: 50%;">' . $row['fname'] . ' ' . $row['lname'].'</td></tr>';
			echo '<tr><td style = "width: 50%;">Particular: </td><td style = "width: 50%;">' . $row['particular'].'</td></tr>';			
			echo '<tr><td style = "width: 50%;">Amount: </td><td style = "width: 50%;"><input class = "form-control" type = "text" name = "pettyamount" value ="' . $row['amount'].'"/></td></tr>';
			echo '<tr><td style = "width: 50%;">Source of Fund</td><td><select name = "source" class = "form-control"><option value = "Eli/Sha">Eli/Sha</option><option value = "Accounting">Accounting</option></select></td></tr>';

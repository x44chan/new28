	...
	if(isset($_GET['pettyac']) && $_GET['pettyac'] == 'a'){
		...
		if($result->num_rows > 0){
			while($row = $result->fetch_assoc()){
				...
				echo '<tr><td style = "width: 30%;">Amount: </td><td style = "width: 50%;"><input id = "petamount" class = "form-control" type = "text" name = "pettyamount" value ="'; if(!is_numeric($row['amount'])){ echo $row['amount']; }else{ echo number_format($row['amount']); };echo'"/></td></tr>';
